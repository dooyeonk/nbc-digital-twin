...
void USplineFollowerComponent::BuildPath()
{
	...
	if (!SplinesComp)
	{
		UE_LOG(LogTemp, Warning, TEXT("SplineFollower: No ALandscapeSplineActor found"));
		return;
	}
	...
	if (!NearestCP)
	{
		UE_LOG(LogTemp, Warning, TEXT("SplineFollower: No control point within SearchRadius"));
		return;
	}
	...
	if (PathPoints.Num() < 3)
	{
		UE_LOG(LogTemp, Warning, TEXT("SplineFollower: Too few path points (%d)"), PathPoints.Num());
		return;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("SplineFollower: %d pts, loop=%s, start=%d"),
...
void USplineFollowerComponent::ResampleCatmullRom()
{
	...
	UE_LOG(LogTemp, Log, TEXT("SplineFollower: Resampled %d -> %d pts"), N, Out.Num());
...
{
	Super::BeginPlay();
	OwnerPawn = Cast<ADTPawn>(GetOwner());
	BuildPath();
}
