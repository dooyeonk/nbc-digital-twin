#include "DT/Public/Sensor/DTLidarSensorComponent.h"

DEFINE_LOG_CATEGORY_STATIC(LogLidarSensor, Log, All);

UDTLidarSensorComponent::UDTLidarSensorComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	PrimaryComponentTick.bStartWithTickEnabled = false;
}

void UDTLidarSensorComponent::BeginPlay()
{
	Super::BeginPlay();
	ApplyPreset(Preset);
	InitializeSensor();
...
void UDTLidarSensorComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	StopScanTimer();
	Super::EndPlay(EndPlayReason);
}

void UDTLidarSensorComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UDTLidarSensorComponent::InitializeSensor()
{
	...
	UE_LOG(LogLidarSensor, Log,
		TEXT("LidarSensor 초기화 : %d ch x %d pts @ %.0f Hz, range %.0f m  [AsyncTrace]"),
		Config.NumChannels, Config.PointsPerChannel,
		Config.RotationRate, Config.MaxRange / 100.0f
	);
}

void UDTLidarSensorComponent::RebuildDirectionCache()
{
...
void UDTLidarSensorComponent::StartScan()
{
	...
	GetWorld()->GetTimerManager().SetTimer(
		ScanTimerHandle,
		this,
		&UDTLidarSensorComponent::OnScanTimer,
		Interval,
		true);
}

void UDTLidarSensorComponent::StopScan()
{
	if (GetWorld())
...
UTexture2D* UDTLidarSensorComponent::GetBevRenderTarget() const
{
	return BevRenderer ? BevRenderer->GetRenderTarget() : nullptr;
}

#if WITH_EDITOR
void UDTLidarSensorComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	const FName PropName = PropertyChangedEvent.GetPropertyName();
	if (PropName == GET_MEMBER_NAME_CHECKED(UDTLidarSensorComponent, Preset))
		ApplyPreset(Preset);
	bDirectionsDirty = true;
}
#endif

void UDTLidarSensorComponent::ApplyPreset(ELidarSensorPreset NewPreset)
{
	Preset = NewPreset;
	bDirectionsDirty = true;
...
void UDTLidarSensorComponent::SetScanRate(float Hz)
{
...
void UDTLidarSensorComponent::RefreshSettings()
{
	bDirectionsDirty = true;
	BevConfig.ViewRange = Config.MaxRange;
...
void UDTLidarSensorComponent::StartScanTimer()
{
	bSensorEnabled = true;
	StartScanTimer();
}

void UDTLidarSensorComponent::StopScanTimer()
{
	bSensorEnabled = false;
	StopScanTimer();
}

void UDTLidarSensorComponent::OnScanTimer()
{
	if (!bSensorEnabled)
		return;
...
void UDTLidarSensorComponent::FireAsyncTraces()
{
...
void UDTLidarSensorComponent::CollectAsyncResults()
{
	UWorld* World = GetWorld();
	if (!World) return;
...
void UDTLidarSensorComponent::SavePointCloudData()
{
